...
- **Push layout into the type system.** If a field has a wire layout quirk (size prefix, endianness, fixed size, sentinel, padding, hidden prefix/suffix, zero-value option, byte-length prefix), describe it via a type-node wrapper — not a sidecar `serialization` / `repr` hint. Covers #2, #3, #10, #15. The IDL should fully describe the serialization without needing to reference Rust-flavored type names or framework-specific assumptions. Wire-layout expressivity reaches full Codama parity as of v1.0.0 (§10.4).
...
- Matches Codama's full wire-layout expressivity via type-node wrappers: `fixedSize`, `hiddenPrefix`, `hiddenSuffix`, `preOffset`, `postOffset`, `sentinel`, `zeroableOption`, `remainderOption`, `sizePrefix`, explicit-size `boolean`, plus struct-field `padding`.
